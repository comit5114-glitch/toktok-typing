...
function openLesson(key) { currentKey = key; currentIndex = 0; $('practiceView').hidden = true; if (key === 'pictures') { $('memoryView').hidden = false; startMemoryGame('beginner'); } else if (key === 'game') { $('arcadeView').hidden = false; resetArcadeGame(); } else { $('lessonView').hidden = false; $('lessonView').classList.toggle('letter-mode', key === 'letters'); $('lessonTitle').textContent = lessons[key].title; $('cheonjiinGuide').hidden = key !== 'letters'; showQuestion(); } window.scrollTo({top:0,behavior:'smooth'}); }
